...
public class GuestListAdapter extends RecyclerView.Adapter<GuestListAdapter.GuestViewHolder> {
     ...
     */
    // DOME (2) Modify the constructor to accept a cursor rather than an integer
    public GuestListAdapter(Context context, Cursor cursor) {
        this.mContext = context;
        // DOME (3) Set the local mCursor to be equal to cursor
        mCursor = cursor;
    }
    ...
    public void onBindViewHolder(GuestViewHolder holder, int position) {
        ...
        int size = mCursor.getInt(mCursor.getColumnIndex(
                WaitlistContract.WaitlistEntry.COLUMN_PARTY_SIZE
        ));
        // DONE) Set the holder's nameTextView text to the guest's name
